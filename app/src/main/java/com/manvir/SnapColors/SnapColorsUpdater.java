    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try {
            if (new updateAv().execute(getPackageManager()).get()) {
                Intent myIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://repo.xposed.info/module/" + PACKAGES.SNAPCOLORS));
    ...
    public static class updateAv extends AsyncTask<PackageManager, Void, Boolean> {
        ...
        protected Boolean doInBackground(PackageManager... packageManager) {
            ...
            try {
                ...
                String currentVersion = packageManager[0].getPackageInfo(PACKAGES.SNAPCOLORS, 0).versionName;
